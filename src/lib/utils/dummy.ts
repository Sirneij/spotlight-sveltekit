...
const markdownString = `{% youtube id="3RlBfUQCiAQ" title="Animation With Svelte" %}

A simple and runnable code example with JavaScript:

\`\`\`js:runnable.js {1, 3-5} runnable:
const add = (a, b) => a + b;
...
\`\`\`html:+page.svelte {2, 3, 5-12, 24-37}:
<script lang="ts">
...
\`\`\`html:+page.svelte {2-3, 6-23, 48-49, 62-63}:
<script lang="ts">
...
\`\`\`html:+page.svelte {3, 12, 18, 26, 49-50, 54}:
<script lang="ts">
  import { onMount } from 'svelte'
...
\`\`\`html:+page.svelte {2-3, 5, 23-33, 54, 62, 74, 88}:
<script lang="ts">
...
const processMarkdown = (markdown: string): string => {
	...
	return markdown.replace(codeBlockRegex, (_, language, meta, code) => {
		let filename = '';
		let lineno = '';
		let runnable = 'false';

		const metaParts = meta.match(/(\S+\.\S+)|(\{[^}]+\})|(runnable)/g);
		if (metaParts) {
			metaParts.forEach((part: string) => {
				if (part.startsWith('{') && part.endsWith('}')) {
					lineno = part.slice(1, -1);
				} else if (part.includes('.')) {
					filename = part;
				} else if (part.toLowerCase() === 'runnable') {
					runnable = 'true';
				}
			});
		}
		...
		return `<pre><code class="language-${language}" data-filename="${filename}" data-lineno="${lineno}" data-runnable="${runnable}">\n${escapeHtml(code.trim())}\n</code></pre>`;
	});
};

const markDowned = async (markdown: string) => {
	return await marked(processMarkdown(markdown));
};

export const article = async () => {
	return await markDowned(markdownString);
};

export const tagsFromServer: Tag[] = [
	{
		id: '1',
		name: 'svelte',
	...
	},
	{
		...
		name: 'sveltekit',
		description:
			'SvelteKit is a framework for building extremely high-performance web applications.'
	},
	{
		id: '3',
		name: 'animation',
		description: 'Animation is the process of creating the illusion of motion and shape change.'
	},
	{
		id: '4',
		name: 'transition',
		description: 'Transition is the process of changing from one state or condition to another.'
	},
	{ id: '5', name: 'motion', description: 'Motion is the process of moving or changing position.' }
];
